    ...
    afterOperation: async ({
      ...
      if (operation === "create") {
        ...
        if (bidVehicle.currentBidAmount < resolvedData.amount) {
          ...
          new Date().getTime() && bidVehicle.event.eventCategory === "open"){
            await Promise.all([
              context.prisma.$executeRaw`UPDATE "Vehicle" set "bidTimeExpire" = "bidTimeExpire" + ${bidVehicle?.event?.extraTime} * INTERVAL '1 MINUTE' WHERE event = ${bidVehicle.event.id} AND "bidTimeExpire" > NOW()`,
