        ...
        if (bidVehicle.currentBidAmount < resolvedData.amount) {
          ...
          new Date().getTime() && bidVehicle.event.eventCategory === "open"){
           const result = await Promise.all([
              context.prisma.$executeRawUnsafe(`UPDATE "Vehicle" set "bidTimeExpire" = "bidTimeExpire" + ${bidVehicle?.event?.extraTime} * INTERVAL '1 MINUTE', "bidStartTime" = "bidStartTime" + ${bidVehicle?.event?.extraTime} * INTERVAL '1 MINUTE' WHERE event = ${eventId} AND "bidStartTime" > NOW()`),
              context.prisma.$executeRawUnsafe(`UPDATE "Vehicle" set "bidTimeExpire" = "bidTimeExpire" + ${bidVehicle?.event?.extraTime} * INTERVAL '1 MINUTE'WHERE event = ${eventId} AND "bidTimeExpire" > NOW() AND "bidStartTime" < NOW()`),
