...
function generateOTP(length: number): string {
    return randomBytes(length^2).readUIntBE(0, 4) // Generates N*8 bits of data
      .toString(10) // Groups by 6-bits and encodes as ascii chars in [A-Za-z0-9+/] and '=' for padding (~8/6 * N chars)
      //.replace(/[^0-9]/g, '') // Removes any '+', '/' (62, 63) and '=' chars as often require escaping (eg. in urls)
...
export async function createAuthOTP(
  ...
  if (item) {
    return { success: true, itemId: item.id, token: generateOTP(8) };
  } else {
